...
test_transactions() {
    ...
    local api_base="http://localhost:8081"
    local tx_data='{"from":"alice","to":"bob","amount":100,"fee":1}'
    ...
    if [ $? -eq 0 ] && echo "$response" | jq -e '.success' > /dev/null 2>&1; then
        tx_hash=$(echo "$response" | jq -r '.data.hash' 2>/dev/null)
...
test_resilience() {
    ...
    if [ -n "$node_to_stop" ]; then
        ...
        response=$(curl -s "http://localhost:8081/api/v1/node/status" 2>/dev/null)
        if [ $? -eq 0 ] && [ -n "$response" ]; then
...
test_consistency() {
    ...
    for pid_file in pids/node_*.pid; do
        if [ -f "$pid_file" ]; then
            ...
            local response=$(curl -s "http://localhost:$((port + 1))/api/v1/node/status" 2>/dev/null)
            if [ $? -eq 0 ] && [ -n "$response" ]; then
                local height=$(echo "$response" | jq -r '.data.height' 2>/dev/null || echo "0")
                heights+=("$height")
            fi
        fi
    done
